    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: theme.background }]}>
      <AppBackground>
        ...
        <View style={styles.header}>
          ...
          <Text style={[styles.headerTitle, { color: theme.text }]}>Place Order</Text>
          <View style={{ width: 36 }} />
        </View>

        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          {/* Your Items */}
          <SectionTitle title={`Your Items (${totalItems})`} theme={theme} />
          ...
          <SectionTitle title="Pickup & Drop Location" theme={theme} />
          <View style={[styles.card, { backgroundColor: theme.card }]}>
            <LocationRow
              label="Pickup From"
          ...
          {/* Schedule */}
          <SectionTitle title="Schedule" theme={theme} />
          <View style={[styles.card, { backgroundColor: theme.card }]}>
            <View style={styles.scheduleRow}>
              <View style={styles.scheduleBlock}>
                <View style={styles.scheduleHeader}>
                  <MaterialCommunityIcons name="calendar-outline" size={14} color={theme.subText} />
                  <Text style={[styles.scheduleHeaderText, { color: theme.subText }]}>Pickup Date</Text>
                </View>
                <View style={styles.pillRow}>
                  {(["Today", "Tomorrow"] as PickupDay[]).map((day) => (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.pill,
                        pickupDay === day ? [styles.pillActive, { backgroundColor: theme.primary }] : [styles.pillInactive, { backgroundColor: theme.border }],
                      ]}
                    >
                      <Text
                        style={[
                          styles.pillText,
                          pickupDay === day ? styles.pillTextActive : [styles.pillTextInactive, { color: theme.subText }],
                        ]}
                      >
                        {day}
                      </Text>
              ...
              <View style={styles.scheduleBlock}>
                <View style={styles.scheduleHeader}>
                  ...
                  <Text style={[styles.scheduleHeaderText, { color: theme.subText }]}>Time Slot</Text>
                </View>
                <View style={styles.pillRow}>
                  {(["10:00 AM", "2:00 PM"] as TimeSlot[]).map((slot) => (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.pill,
                        timeSlot === slot ? [styles.pillActive, { backgroundColor: theme.primary }] : [styles.pillInactive, { backgroundColor: theme.border }],
                      ]}
                    >
                      <Text
                        style={[
                          styles.pillText,
                          timeSlot === slot ? styles.pillTextActive : [styles.pillTextInactive, { color: theme.subText }],
                        ]}
                      >
                        {slot}
                      </Text>
          ...
          <View style={[styles.card, { backgroundColor: theme.card }]}>
            <View style={styles.billRow}>
              <Text style={[styles.billLabel, { color: theme.subText }]}>Items ({totalItems})</Text>
              <Text style={[styles.billValue, { color: theme.text }]}>₹{itemsTotal}</Text>
            </View>
            <View style={styles.billRow}>
              <Text style={[styles.billLabel, { color: theme.subText }]}>Delivery</Text>
              <Text style={[styles.billFree, { color: theme.primary }]}>FREE</Text>
            </View>
          ...
          {/* ✅ Terms & Conditions checkbox removed */}
        </ScrollView>

        {/* Sticky CTA – always enabled because T&C checkbox removed */}
